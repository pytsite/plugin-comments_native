define(['jquery', 'http-api', 'assetman'], function ($, httpApi, assetman) {
    return function (widget) {
        ...
        var commentMaxDepth = parseInt(em.data('maxDepth'));
        ...
        function createReplyForm(parentCommentUid) {
            ...
            form.find('.comment-body').attr('placeholder', t('comments_native@enter_your_reply'));
        ...
        function onFormSubmit(e, form) {
            ...
            httpApi.post(em.data('commentSubmitEp'), {
        ...
        function onBtnDeleteClick(btn, commentSlot) {
            if (confirm(t('comments_native@confirm_comment_deletion'))) {
                httpApi.delete(em.data('commentSubmitEp') + '/' + commentSlot.data('uid')).done(function (r) {
                    if (r['status']) {
                        commentSlot.find('> .content').html('');
                        commentSlot.find('> .header').html(t('comments_native@comment_was_deleted'));
        ...
        function onBtnReportClick(commentUid) {
            if (confirm(t('comments_native@confirm_comment_report'))) {
                httpApi.post(em.data('commentReportEp') + '/' + commentUid).done(function () {
                    alert(t('comments_native@comment_report_confirmed'));
                });
            }
        }
        ...
        function createCommentSlot(comment) {
            ...
            if (comment['status'] === 'published') {
                ...
                var btnReport = $('<div class="report"><a class="report-btn" href="#" title="' +
                    t('comments_native@report') + '"><i class="fa fa-fw fa-flag"></i></a></div>');
                ...
                if (comment['permissions']['delete']) {
                    var btnDelete = $('<div class="delete"><a class="delete-btn" href="#" title="' +
                        t('comments_native@delete') + '"><i class="fa fa-fw fa-trash-o"></i></a></div>');
                ...
                header.html(t('comments_native@comment_was_deleted'));
            }
            ...
            if (comment['status'] !== 'deleted' && commentCreatePermission && parseInt(comment['depth']) < commentMaxDepth) {
                var actions = $('<div class="actions">');
                content.append(actions);

                // 'Reply' button
                var btnReply = $('<div class="reply"><a class="reply-btn" href="#"><i class="fa fa-fw fa-reply"></i>&nbsp;' +
                    t('comments_native@reply') + '</a></div>');
        ...
        assetman.loadCSS('plugins.comments_native@css/comments-native-widget.css');

        // Load comments
        httpApi.get(em.data('commentsLoadEp'), {
            thread_uid: threadUid,
            skip: commentsLoadOffset
        }).success(function (r) {
            commentsList.html('<p class="no-comments-yet">' + t('comments_native@no_comments_yet') + '</p>');
